    ...
    <div className="bg-white rounded-[2rem] shadow-xl overflow-hidden border border-gray-100 h-full flex flex-col hover:shadow-2xl transition-shadow duration-300">
      ...
      <div className="p-6 sm:p-8 space-y-8 flex-grow">
        ...
        <div>
          ...
          <p className="text-gray-600 text-sm leading-relaxed bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
            {data.timeline}
          </p>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {data.considerations.map((effect, idx) => (
              <span 
                key={idx} 
                className="px-3 py-1.5 bg-amber-50 text-amber-800 border border-amber-100 rounded-lg text-xs font-bold"
              >
                {effect}
              </span>
            ))}
